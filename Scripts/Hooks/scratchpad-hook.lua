local function loadScratchpad()
    ...
    function formatCoord(type, isLat, d)
		...
		elseif type == "DM" then -- Degree Minutes -- @Alex : add gazelle coordinates
            s = math.floor((s * 10)/60)
            return string.format('%s %2d°%.2d.%1d', h, g, m, s)
    ...
    local function coordsType()
        ...
        elseif ac == "F-16C_50" or ac == "M-2000C" then
            return "DDM", false
		elseif ac == "SA342L" or ac == "SA342M" or ac == "SA342Mistral" or ac == "SA342Minigun" then -- @Alex : add gazelle DM type
            return "DM", true
        else
            return nil, false
        end
    ...
    local function insertCoordinates()
        ...
        if type == nil or type == "DMS" then
            result = result .. "DMS" .. " : ".. formatCoord("DMS", true, lat) .. " | " .. formatCoord("DMS", false, lon) .. "\n"
        end
        if type == nil or type == "DDM" then
            result = result .. "DDM" .. " : ".. formatCoord("DDM", true, lat) .. " | " .. formatCoord("DDM", false, lon) .. "\n"
        end
		if type == nil or type == "DM" then -- @Alex : add gazelle DM type
            result = result .. "DM" .. " : "..formatCoord("DM", true, lat) .. " | " .. formatCoord("DM", false, lon) .. "\n"
        end
        if type == nil or includeMgrs then
            result = result .. "MGRS" .. " : ".. mgrs .. "\n"
        end
